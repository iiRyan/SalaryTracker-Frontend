...
export default function Months() {
  const { data, isFetching, isSuccess, isError } = api.useGetSalariesQuery(undefined, {
    refetchOnMountOrArgChange: true, // This forces refetching the data on mount
  });

  const [deleteMonth] = api.useDeleteSalaryMutation();

  const handleDelete = (id) => {
    deleteMonth({ recordId: id }).unwrap();
  };
  ...
  return (
    <div className="min-h-screen bg-customBlack">
      <div className="container mx-auto max-w-6xl drop-shadow-lg text-gray-800 pt-20 ">
            ...
            <div className="overflow-hidden ">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 p-8">
                {data.map((salary) => (
                  <div
                    key={salary.id}
                    className="bg-[#18191A] border border-gray-700 rounded-xl shadow-lg p-6 transform hover:scale-105 transition duration-300 relative"
                  >
                    <Link
                      to={`/read/${salary.id}`}
                      state={{
                        salaryId: salary.amount,
                        amount: salary.amount,
                        description: salary.description,
                      }}
                    >
                      <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-[#3A3B3C] to-purple-600 rounded-t-xl"></div>
                      <h2 className="text-xl font-semibold text-white mb-2">
                        {capitalizeFirstLetter(salary.month)}
                      </h2>
                    ...
                    <button onClick={() => handleDelete(salary.id)}>
                          <svg
